...
- **적합도 격차 (Fitness Gap):** $$\text{Fitness Gap} = \text{Avg}(\text{Target Scores}) - \text{Avg}(\text{Noise Scores})$$
  > 이 격차가 커질수록 수식이 정상적으로 작동한다는 증거입니다. 본 모델은 최적화를 통해 **12.45점**이라는 높은 격차를 확보했습니다.
...
- **현실적 상한선:** 수식이 10점에 근접할 수는 있지만, 이론적으로 무한대의 위험이 발생해야만 10점이 됩니다. 이는 보안 시스템의 **마지노선**을 제공하여 10점 만점 남발을 막아줍니다.
