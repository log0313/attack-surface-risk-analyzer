...
def _preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # flag_cwe 생성 (이제 FEATURES에 포함되어 학습에 사용됨)
...
def run_single_round(df: pd.DataFrame, seed: int) -> dict:
    ...
    # 룰 기반 보정 제거, 모델 자체 확률값 바로 사용
    prob = pipeline.predict_proba(X_test)[:, 1]

    threshold, precision, recall = find_best_threshold(
        prob, y_test, CONFIG["target_recall"]
    )
    ...
    return {
        ...
        "X_test"       : X_test,
        "y_test"       : y_test,
        "prob"         : prob,  # adjusted_prob 대신 prob 저장
    }
...
def search_best_model(client: OpenSearch):
    print("=" * 60)
    print(f" 목표: KEV Recall ≥ {CONFIG['target_recall']} | 최대 {CONFIG['max_rounds']} 라운드")
    print(f" 고정 weight={CONFIG['kev_weight']} | 학습 피처(CWE 포함): {FEATURES}")
    print("=" * 60)
...
def print_final_report(result: dict):
    y_pred = (result["prob"] >= result["threshold"]).astype(int)
    ...
    # 역산(Unscaling)하여 원래 데이터(Raw) 기준의 계수와 Bias 계산
    ...
    # 스캐너에 그대로 복붙할 수 있는 파이썬 수식 생성 (보정 로직 제거됨)
    print("\n💡 [최종 스캐너 적용 수식 (Python)]")
    ...
    print(f"1. {formula}")
    print(f"2. final_prob = 1 / (1 + np.exp(-z))  # Sigmoid 적용")
    print("=" * 60)
...
def plot_results(result: dict, history: list):
    ...
    # ── 1. PR Curve ──
    ax = axes[0][0]
    prec, rec, thr = precision_recall_curve(result["y_test"], result["prob"])
    ...
    score_nonkev = prob[y_test == 0]
    score_kev    = prob[y_test == 1]
    ...
    ax.set_xlabel("위협도 점수 (Probability)")
